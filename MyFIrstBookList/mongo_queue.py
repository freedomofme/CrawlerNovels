...
class MongoQueue:
    ...
    def __init__(self, client=None, timeout=300, expires=timedelta(hours=36)):
        ...
        self.db.crawl_queue.create_index('timestamp', expireAfterSeconds=expires.total_seconds())
    ...
    def repush(self, url):
        ...
        if (record.has_key('retry')):
            ...
            self.db.crawl_queue.update({'_id': url}, {'$set': {'status': self.OUTSTANDING, 'retry': 1}})
